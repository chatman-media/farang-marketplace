...
- **PgAdmin**: `http://localhost:5050` (опционально, используйте `--profile tools`)
...
Этот проект использует Turbo для управления монорепозиторием со следующей структурой workspace:
...
Пакет `@marketplace/shared-types` содержит все TypeScript интерфейсы и типы, используемые на платформе:
...
Каждый сервис может иметь дополнительные переменные окружения. Проверьте файл `.env.example` в директории каждого сервиса.

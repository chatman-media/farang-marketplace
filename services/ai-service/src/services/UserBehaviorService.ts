...
export class UserBehaviorService {
  ...
  async trackBehavior(
    ...
    try {
      ...
      await db.insert(userBehaviors).values({
        ...
        latitude: behavior.location?.latitude?.toString(),
        longitude: behavior.location?.longitude?.toString(),
        city: behavior.location?.city,
        country: behavior.location?.country,
        deviceType: behavior.device?.type,
        deviceOs: behavior.device?.os,
        deviceBrowser: behavior.device?.browser,
  ...
  async analyzeUserBehavior(userId: string): Promise<UserInsight[]> {
    try {
      ...
      if (dbBehaviors.length === 0) return [];

      // Convert to UserBehavior type
      const behaviors = dbBehaviors.map(b => ({
        id: b.id,
        userId: b.userId,
        ...
        device: b.deviceType
          ? {
              type: b.deviceType as any,
              os: b.deviceOs || undefined,
              browser: b.deviceBrowser || undefined,
            }
          : undefined,
      }));
      ...
      const patterns = this.extractBehaviorPatterns(behaviors as any);

      // Generate insights using AI
      const aiInsights = await this.generateAIInsights(userId, behaviors as any, patterns);
      insights.push(...aiInsights);

      // Generate statistical insights
      const statInsights = this.generateStatisticalInsights(userId, behaviors as any, patterns);
...
Focus on actionable business insights and market opportunities.
  ...
  async getUserBehaviors(userId: string, options?: { limit?: number }): Promise<UserBehavior[]> {
    ...
    const dbBehaviors = options?.limit ? await baseQuery.limit(options.limit) : await baseQuery;

    // Convert to UserBehavior type
    return dbBehaviors.map(b => ({
      id: b.id,
      userId: b.userId,
      action: b.action as any,
      entityType: b.entityType as any,
   ...
   */
  async getMarketInsights(_filters?: {
    type?: string;
    category?: string;
    location?: string;
    minConfidence?: number;
  }): Promise<MarketInsight[]> {
    const behaviors = await db.select().from(userBehaviors).orderBy(desc(userBehaviors.timestamp)).limit(10000);
  ...
  async generateUserInsights(
    ...
    if (options.type === 'engagement') {
      const avgDuration =
        behaviors.reduce((sum: number, b: any) => sum + (b.metadata?.['duration'] || 0), 0) / behaviors.length;
      insights.push({
        userId,
        type: 'behavior',
  ...
  async analyzeMarketTrends(options: {
    ...
    if (options.type === 'category') {
      const categories = allBehaviors.map((b: any) => b.metadata?.['category']).filter(Boolean);
    ...
    if (options.type === 'location') {
      const locations = allBehaviors.map((b: any) => b.metadata?.['location']).filter(Boolean);
    ...
    if (options.type === 'price' && options.category) {
      const categoryBehaviors = allBehaviors.filter((b: any) => b.metadata?.['category'] === options.category);
      const prices = categoryBehaviors.map((b: any) => b.metadata?.['price']).filter(Boolean) as number[];
  ...
  > {
    ...
    if (options.algorithm === 'behavior') {
      ...
      allBehaviors.forEach(([userId, behaviors]: [string, any[]]) => {
        const actions = behaviors.map((b: any) => b.action);
        const topAction = this.getMostFrequent(actions);
        if (topAction) userActions.set(userId, topAction as string);
      });
    ...
    if (options.algorithm === 'preference') {
      ...
      allBehaviors.forEach(([userId, behaviors]: [string, any[]]) => {
        const categories = behaviors.map((b: any) => b.metadata?.['category']).filter(Boolean);
        const topCategory = this.getMostFrequent(categories);
        if (topCategory) userCategories.set(userId, topCategory as string);
      });

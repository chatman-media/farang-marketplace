...
import type { CreatePaymentRequest, PaymentSearchFilters } from "../services/PaymentService.js"
...
export class PaymentController {
  ...
  createPayment = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (!this.paymentService.validatePaymentMethod(paymentRequest.paymentMethod, req.body)) {
        res.status(400).json({
  ...
  processTonPayment = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const payment = await this.paymentService.processTonPayment(paymentId, fromAddress)
      ...
      res.status(500).json({
        ...
        message: error instanceof Error ? error.message : "Failed to process TON payment",
        timestamp: new Date().toISOString(),
      })
    }
  }
  ...
  getPayment = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (payment.payerId !== userId && payment.payeeId !== userId && req.user?.role !== "admin") {
        res.status(403).json({
          error: "Forbidden",
  ...
  searchPayments = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const result = await this.paymentService.searchPayments(filters, page, limit)
   ...
   */
  getPaymentTransactions = async (req: Request, res: Response): Promise<void> => {
    try {
      const { paymentId } = req.params
      const userId = req.user?.id

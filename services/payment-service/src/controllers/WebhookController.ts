...
export class WebhookController {
  ...
  handleTonWebhook = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (
        !this.tonService.verifyWebhookSignature(
          payload,
          signature,
          webhookSecret
        )
  ...
  private async handleTransactionConfirmed(transaction: any): Promise<void> {
    try {
      ...
      console.log(
        `Transaction confirmed: ${hash} (${confirmations} confirmations)`
      );
    } catch (error) {
  ...
  private async updatePaymentConfirmations(
    ...
    try {
      // This would be implemented in PaymentService
      console.log(
        `Updating payment ${paymentId} confirmations to ${confirmations}`
      );
    } catch (error) {
  ...
  handleRefundWebhook = async (req: Request, res: Response): Promise<void> => {
    try {
      const { payment_id, refund_amount, refund_reason, transaction_hash } =
        req.body;

      if (!payment_id || !refund_amount) {
        res
          .status(400)
          .json({ error: 'Payment ID and refund amount required' });
        return;
      }

...
export class TonService {
  ...
  constructor() {
    this.network = (process.env.TON_NETWORK as "mainnet" | "testnet") || "testnet"
  ...
  async getWalletInfo(address?: string): Promise<TonWalletInfo> {
    try {
      ...
      const state = await this.client.getContractState(Address.parse(walletAddress))
   ...
   */
  async calculateTonAmount(fiatAmount: number, fiatCurrency: string = "USD"): Promise<string> {
    try {

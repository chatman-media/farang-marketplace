...
export const defaultAIConfig: AIProviderConfig = {
  ...
  // Provider Selection
  defaultProvider: (process.env.DEFAULT_AI_PROVIDER as AIProvider) || AIProvider.DEEPSEEK,
  fallbackProviders: [AIProvider.DEEPSEEK, AIProvider.OPENAI, AIProvider.CLAUDE],
...
export function validateAIConfig(config: AIProviderConfig): {
  ...
  const hasDeepSeek = config.deepseek?.apiKey && config.deepseek.apiKey.length > 0
  const hasClaude = config.claude?.apiKey && config.claude.apiKey.length > 0

  if (!hasOpenAI && !hasDeepSeek && !hasClaude) {
    errors.push("At least one AI provider must be configured with a valid API key")
  }
  ...
  if (!config.defaultProvider) {
    ...
    if (!defaultProviderConfigured) {
      errors.push(`Default provider ${config.defaultProvider} is not properly configured`)
    }
  }
  ...
  Object.entries(config.rateLimits).forEach(([provider, limits]) => {
    if (limits.requestsPerMinute <= 0) {
      warnings.push(`Rate limit for ${provider} requests per minute should be positive`)
    }
    if (limits.requestsPerDay <= 0) {
      warnings.push(`Rate limit for ${provider} requests per day should be positive`)
    }
    if (limits.requestsPerDay < limits.requestsPerMinute * 60) {
      warnings.push(`Daily rate limit for ${provider} may be too low compared to per-minute limit`)
    }
  })
...
export const providerStrategies = {
  ...
  performanceOptimized: {
    ...
    fallbackProviders: [AIProvider.OPENAI, AIProvider.CLAUDE, AIProvider.DEEPSEEK],
  },

  // Quality-optimized: Use highest quality provider (Claude 4 Sonnet)
  qualityOptimized: {
    defaultProvider: AIProvider.CLAUDE,
    fallbackProviders: [AIProvider.CLAUDE, AIProvider.OPENAI, AIProvider.DEEPSEEK],
  },
...
export function estimateOperationCost(provider: AIProvider, estimatedTokens: number): number {

...
export class AIService {
  ...
  private async executeWithFallback<T>(
    ...
    for (const providerType of providersToTry) {
      ...
      if (!this.checkRateLimit(providerType)) {
        console.warn(
          `Rate limit exceeded for ${providerType}, trying next provider`
        );
        continue;
      }
  ...
  async enhanceSearch(
    ...
    try {
      ...
      const suggestions = await this.executeWithFallback(
        (provider) =>
          provider.generateSuggestions(query.query, query.userContext),
        preferredProvider
      );
  ...
  async analyzeQuery(
    ...
    return this.executeWithFallback(
      (provider) => provider.analyzeQuery(query),
      preferredProvider
    );
  }
  ...
  async getCostEstimate(
    ...
    for (const [provider, instance] of this.providers) {
      if (
        'calculateCost' in instance &&
        typeof (instance as any).calculateCost === 'function'
      ) {

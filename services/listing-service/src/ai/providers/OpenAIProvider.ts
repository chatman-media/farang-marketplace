  ...
  protected async makeRequest(prompt: string, options?: any): Promise<string> {
    ...
    try {
      ...
      if (!data.choices || !data.choices[0] || !data.choices[0].message) {
        throw this.createAIError("Invalid response format from OpenAI", "OPENAI_INVALID_RESPONSE")
      }

      return data.choices[0].message.content
    } catch (error) {
      if (error instanceof Error && error.name === "AbortError") {
        throw this.createAIError("OpenAI request timeout", "OPENAI_TIMEOUT", true)
      }

      if (error instanceof Error && error.message.includes("fetch")) {
        throw this.createAIError("Network error connecting to OpenAI", "OPENAI_NETWORK_ERROR", true)
      }
    ...
    const functions = [
      {
        ...
        description: "Enhance a marketplace search query with better keywords and filters",
        parameters: {
          type: "object",
          properties: {
            enhancedQuery: {
...
Return a JSON array of enhanced queries in the same order.`
  ...
  async streamSuggestions(partialQuery: string): Promise<AsyncGenerator<string, void, unknown>> {

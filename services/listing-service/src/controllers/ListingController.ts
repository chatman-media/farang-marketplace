...
export class ListingController {
  ...
  static createVehicleValidation = [
    ...
    body('specifications.transmission').isIn([
      'manual',
      'automatic',
      'cvt',
      'semi_automatic',
    ...
    try {
      ...
      const filters: VehicleSearchFilters = {
        ...
        fuelType: req.query.fuelType
          ? ((Array.isArray(req.query.fuelType)
              ? req.query.fuelType
        ...
        seatingCapacity:
          req.query.minSeats || req.query.maxSeats
            ? {
                min: req.query.minSeats
                  ? parseInt(req.query.minSeats as string)
                  : 1,
                max: req.query.maxSeats
                  ? parseInt(req.query.maxSeats as string)
                  : 50,
              }
            : undefined,
        yearRange:
      ...
      const result = await this.listingService.searchVehicleListings(
        filters,
        page,
        limit
      );
  ...
  searchProductListings = async (req: Request, res: Response) => {
    try {
      ...
      const filters: ProductSearchFilters = {
        ...
        seller: {
          verified: req.query.sellerVerified
            ? req.query.sellerVerified === 'true'
            : undefined,
      ...
      const result = await this.listingService.searchProductListings(
        filters,
        page,
        limit
      );
  ...
  updateListingStatus = async (req: Request, res: Response) => {
    try {
      ...
      const updated = await this.listingService.updateListingStatus(
        id,
        status,
        ownerId
      );

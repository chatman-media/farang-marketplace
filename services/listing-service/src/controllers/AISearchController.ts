...
export class AISearchController {
  ...
  static enhancedSearchValidation = [
    ...
    body('userContext.location.latitude')
      .optional()
      .isFloat({ min: -90, max: 90 }),
    body('userContext.location.longitude')
      .optional()
  ...
  enhancedSearch = async (req: Request, res: Response) => {
    try {
      ...
      const searchResponse = await this.aiService.enhanceSearch(
        searchQuery,
        preferredProvider
      );
  ...
  matchServices = async (req: Request, res: Response) => {
    try {
      ...
      const matches = await this.aiService.matchServices(
        serviceMatchingRequest,
        preferredProvider
      );

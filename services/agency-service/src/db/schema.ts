...
export const agencies = pgTable(
  'agencies',
  {
    ...
    totalReviews: decimal('total_reviews', { precision: 10, scale: 0 }).default(
      '0'
    ),
    totalOrders: decimal('total_orders', { precision: 10, scale: 0 }).default(
      '0'
    ),
  ...
  (table) => ({
    ...
    verificationStatusIdx: index('agencies_verification_status_idx').on(
...
export const agencyServices = pgTable(
  'agency_services',
  {
    ...
    pricingModel: varchar('pricing_model', { length: 50 })
      .notNull()
      .default('fixed'), // fixed, hourly, per_item
...
export const serviceAssignments = pgTable(
  ...
  (table) => ({
    ...
    listingIdIdx: index('service_assignments_listing_id_idx').on(
      table.listingId
    ),
    bookingIdIdx: index('service_assignments_booking_id_idx').on(
      table.bookingId
    ),
    statusIdx: index('service_assignments_status_idx').on(table.status),
    assignedAtIdx: index('service_assignments_assigned_at_idx').on(
      table.assignedAt
    ),
  })
);
...
export const commissionPayments = pgTable(
  ...
  (table) => ({
    ...
    serviceAssignmentIdIdx: index(
      'commission_payments_service_assignment_id_idx'
    ).on(table.serviceAssignmentId),
...
export type ServiceAssignmentStatusType =
  (typeof serviceAssignmentStatus.enumValues)[number];

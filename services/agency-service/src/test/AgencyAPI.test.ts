  ...
  describe('Agency Management Endpoints', () => {
    ...
    it('should validate agency search endpoint structure', () => {
      const searchAgenciesEndpoint = {
        ...
        requiresAuth: false,
        queryParams: {
          status: { optional: true, enum: ['pending', 'active', 'suspended', 'inactive', 'rejected'] },
          verificationStatus: { optional: true, enum: ['pending', 'verified', 'rejected', 'expired'] },
    ...
    it('should validate service creation endpoint structure', () => {
      const createServiceEndpoint = {
        ...
        validation: {
          name: { required: true, minLength: 2, maxLength: 255 },
          description: { required: true, minLength: 10, maxLength: 2000 },
          category: { required: true, enum: ['delivery', 'emergency', 'maintenance', 'insurance', 'cleaning', 'security', 'transportation', 'legal', 'financial', 'marketing', 'consulting', 'other'] },
          basePrice: { required: true, type: 'number', min: 0 },
          pricingModel: { required: true, enum: ['fixed', 'hourly', 'per_item', 'percentage'] },
        },
        responses: {
    ...
    it('should validate service search endpoint structure', () => {
      const searchServicesEndpoint = {
        ...
        queryParams: {
          ...
          page: { optional: true, type: 'number', min: 1 },
          limit: { optional: true, type: 'number', min: 1, max: 100 },
          sortBy: { optional: true, enum: ['name', 'basePrice', 'createdAt', 'category'] },
    ...
    it('should validate bulk price update endpoint', () => {
      const bulkUpdatePricesEndpoint = {
        ...
        validation: {
          ...
          priceMultiplier: { required: true, type: 'number', min: 0.1, max: 10 },
        },
        responses: {
  ...
  describe('Service Assignment Endpoints', () => {
    ...
    it('should validate assignment status update endpoint', () => {
      const updateStatusEndpoint = {
        ...
        validation: {
          id: { required: true, format: 'uuid' },
          status: { required: true, enum: ['active', 'paused', 'completed', 'cancelled'] },
    ...
    it('should validate find matching agencies endpoint', () => {
      const findMatchesEndpoint = {
        ...
        validation: {
          ...
          serviceType: { required: true, enum: ['delivery', 'emergency', 'maintenance', 'insurance', 'cleaning', 'security', 'transportation', 'legal', 'financial', 'marketing', 'consulting', 'other'] },
    ...
    it('should validate auto-assign endpoint', () => {
      const autoAssignEndpoint = {
        ...
        validation: {
          ...
          bookingId: { required: true, format: 'uuid' },
          serviceType: { required: true, enum: ['delivery', 'emergency', 'maintenance'] },
        },
        responses: {
  ...
  describe('Authentication and Authorization', () => {
    ...
    it('should validate agency ownership checks', () => {
      const ownershipScenarios = [
        { userRole: 'admin', userAgencyId: null, targetAgencyId: 'any', shouldAllow: true },
        { userRole: 'agency_owner', userAgencyId: 'agency-123', targetAgencyId: 'agency-123', shouldAllow: true },
        { userRole: 'agency_owner', userAgencyId: 'agency-123', targetAgencyId: 'agency-456', shouldAllow: false },
        { userRole: 'user', userAgencyId: null, targetAgencyId: 'agency-123', shouldAllow: false },
      ];

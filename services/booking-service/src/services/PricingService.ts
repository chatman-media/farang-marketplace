...
export class PricingService {
  ...
  async calculateBookingPrice(
    ...
    const subtotal =
      adjustedPrice + cleaningFee + platformFee + paymentProcessingFee;
  ...
  async calculateServicePrice(
    ...
    switch (servicePrice.priceType) {
      ...
      default:
        basePrice =
          servicePrice.fixedPrice || servicePrice.hourlyRate * durationInHours;
    }

    // 4. Apply delivery method surcharge
    const deliveryMultiplier = this.getDeliveryMethodMultiplier(
  ...
  private convertDurationToHours(duration: {
    value: number;
    unit: string;
  }): number {

...
export type ServiceBookingType =
  (typeof serviceBookingTypeEnum.enumValues)[number];
...
export const bookings = pgTable(
  'bookings',
  {
    ...
    serviceFees: decimal('service_fees', { precision: 10, scale: 2 }).default(
      '0'
    ),
...
export const serviceBookings = pgTable(
  ...
  (table) => ({
    ...
    providerIdIdx: index('service_bookings_provider_id_idx').on(
      table.providerId
    ),
    scheduledDateIdx: index('service_bookings_scheduled_date_idx').on(
      table.scheduledDate
    ),
    serviceTypeIdx: index('service_bookings_service_type_idx').on(
      table.serviceType
    ),
  })
);
...
export const bookingStatusHistory = pgTable(
  ...
  (table) => ({
    ...
    changedAtIdx: index('booking_status_history_changed_at_idx').on(
      table.changedAt
    ),
  })
);
...
export const availabilityConflicts = pgTable(
  ...
  (table) => ({
    listingIdIdx: index('availability_conflicts_listing_id_idx').on(
      table.listingId
    ),
    dateRangeIdx: index('availability_conflicts_date_range_idx').on(
      table.startDate,
      table.endDate
    ),
  })
...
export const availabilityConflictsRelations = relations(

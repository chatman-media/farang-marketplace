...
const quickEstimateValidation = [
  ...
  query('type')
    .isIn(['accommodation', 'service'])
    .withMessage('Type must be either accommodation or service'),
  query('duration')
    .optional()
    .isInt({ min: 1 })
  ...
  body().custom((value, { req }) => {
    ...
    if (type === 'accommodation') {
      if (!value.listingId || !value.checkIn || !value.guests) {
        throw new Error(
          'For accommodation: listingId, checkIn, and guests are required'
        );
      }
      ...
      if (
        ...
        throw new Error(
          'For service: listingId, serviceType, duration, and deliveryMethod are required'
        );
      }
    }
...
const comparePricingValidation = [
  body('options')
    ...
    .withMessage('Options must be an array with 1-10 items'),
  body('options.*.listingId')
    .isUUID()
    .withMessage('Each option must have a valid listing ID'),
  body('options.*.type')
    .isIn(['accommodation', 'service'])
    .withMessage('Each option must have a valid type'),
];
...
router.post(
  ...
  bookingPriceValidation,
  pricingController.calculateBookingPrice
);
...
router.post(
  ...
  pricingController.calculateServicePrice
);
...
router.get(
  ...
  pricingController.getQuickEstimate
);
...
router.post(
  '/breakdown',
  optionalAuthMiddleware,
  pricingBreakdownValidation,
  pricingController.getPricingBreakdown
);
...
router.post(
  '/compare',
  optionalAuthMiddleware,
  comparePricingValidation,
  pricingController.comparePricing
);

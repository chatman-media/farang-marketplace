...
export class BookingController {
  ...
  createServiceBooking = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const providerId = await this.getProviderIdFromListing(
        serviceBookingRequest.listingId
      );
  ...
  getServiceBooking = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const serviceBooking =
        await this.bookingService.getServiceBookingById(bookingId);
      ...
      // Check if user has access to this booking
      if (
        serviceBooking.guestId !== userId &&
        serviceBooking.hostId !== userId
      ) {
        res.status(403).json({
          error: 'Forbidden',
  ...
  updateBookingStatus = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (
        existingBooking.guestId !== userId &&
        existingBooking.hostId !== userId
      ) {
        res.status(403).json({
          error: 'Forbidden',
      ...
      const statusCode = error.message.includes('Invalid status transition')
        ? 400
        : 500;
  ...
  getBookingStatusHistory = async (
    ...
    try {
      ...
      const history =
        await this.bookingService.getBookingStatusHistory(bookingId);
  ...
  private async getHostIdFromListing(
    listingId: string
  ): Promise<string | null> {
  ...
  private async getProviderIdFromListing(
    listingId: string
  ): Promise<string | null> {
    // Mock implementation - would fetch from listing service
    // In real implementation, this would make an HTTP request to listing service

...
export interface OAuthProvider {
  ...
  getUserProfile(request: OAuthLoginRequest | LinkSocialAccountRequest): Promise<SocialProfile>
}
...
export class OAuthService {
  ...
  private initializeProviders() {
    if (this.config.google) {
      this.providers.set(AuthProvider.GOOGLE, new GoogleOAuthProvider(this.config.google))
    }
    if (this.config.apple) {
      this.providers.set(AuthProvider.APPLE, new AppleOAuthProvider(this.config.apple))
    }
    if (this.config.tiktok) {
      this.providers.set(AuthProvider.TIKTOK, new TikTokOAuthProvider(this.config.tiktok))
    }
    if (this.config.telegram) {
      this.providers.set(AuthProvider.TELEGRAM, new TelegramOAuthProvider(this.config.telegram))
    }
    if (this.config.line) {
      this.providers.set(AuthProvider.LINE, new LineOAuthProvider(this.config.line))
    }
    if (this.config.whatsapp) {
      this.providers.set(AuthProvider.WHATSAPP, new WhatsAppOAuthProvider(this.config.whatsapp))
    }
  }
  ...
  async authenticateWithProvider(request: OAuthLoginRequest): Promise<AuthResponse> {
    ...
    if (!provider) {
      throw new Error(`Provider ${request.provider} is not configured or supported`)
    }
    ...
    if (!user) {
      ...
      if (socialProfile.email) {
        const existingUser = await this.userService.getUserByEmail(socialProfile.email)
        if (existingUser) {
    ...
    const userEntity = await this.userService.validateUserCredentials(user.email!, "oauth-login")
    if (!userEntity) {
      throw new Error("Failed to retrieve user entity for token generation")
    }

    const accessToken = (this.authService as any).generateAccessToken(userEntity)
    const refreshToken = (this.authService as any).generateRefreshToken(userEntity)
  ...
  async unlinkSocialAccount(userId: string, request: UnlinkSocialAccountRequest): Promise<void> {
    const user = await this.userService.getUserById(userId)
    if (!user) {
      throw new Error("User not found")
    }
    ...
    const profileExists = socialProfiles.find((p: SocialProfile) => p.provider === request.provider)
    if (!profileExists) {
  ...
  private async findUserBySocialProfile(_socialProfile: SocialProfile): Promise<User | null> {
...
export class TelegramOAuthProvider extends BaseOAuthProvider {
  ...
  private verifyTelegramAuth(data: any): boolean {
    ...
    const secretKey = createHash("sha256").update(this.config.botToken).digest()

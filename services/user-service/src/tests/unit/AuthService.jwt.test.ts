...
class MockUserService {
  async createUser(userData: any) {
    const userId =
      userData.email === 'newuser@example.com' ? 'user-456' : 'user-123';
    return {
      id: userId,
...
describe('AuthService JWT Integration Tests', () => {
  ...
  describe('JWT Token Generation and Validation', () => {
    ...
    it('should reject expired tokens', async () => {
      ...
      await expect(
        authService.validateAccessToken(expiredToken)
      ).rejects.toThrow();
    });
    ...
    it('should reject tokens with wrong secret', async () => {
      ...
      await expect(
        authService.validateAccessToken(wrongSecretToken)
    ...
    it('should reject malformed tokens', async () => {
      ...
      await expect(
        authService.validateAccessToken(malformedToken)
      ).rejects.toThrow('Invalid access token');
    });
  });

...
describe.skip("OAuthService", () => {
  ...
  describe("authenticateWithProvider", () => {
    ...
    it("should create new user when no existing user found", async () => {
      ...
      mockUserService.validateUserCredentials.mockResolvedValue(mockUserEntity as any)
  ...
  describe("linkSocialAccount", () => {
    ...
    it("should throw error if social account already linked to another user", async () => {
      ...
      ;(newOAuthService as any).findUserBySocialProfile = vi.fn().mockResolvedValue(existingUser)
  ...
  describe("unlinkSocialAccount", () => {
    it("should throw error if user not found", async () => {
      ...
      await expect(oauthService.unlinkSocialAccount("user-123", request)).rejects.toThrow("User not found")
    })
...
describe("TelegramOAuthProvider", () => {
  ...
  describe("getUserProfile", () => {
    it("should throw error if no Telegram data provided", async () => {
      ...
      await expect(provider.getUserProfile(request as any)).rejects.toThrow("Telegram data is required")
    })
    ...
    it("should throw error if Telegram auth data is invalid", async () => {
      ...
      await expect(provider.getUserProfile(request as any)).rejects.toThrow("Invalid Telegram authentication data")
    })
  })
})
